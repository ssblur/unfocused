...
object ClientScreenRegistrationEvent: SimpleEvent<ClientScreenRegistrationEvent.Context<in AbstractContainerMenu, Nothing>>(true) {
  ...
  data class Context<T: AbstractContainerMenu, U>(
    val menu: MenuType<T>,
  ...
  fun <T: AbstractContainerMenu, U> ModInitializer.registerScreen(menu: MenuType<T>, supplier: ScreenConstructor<T, U>)
    where U: Screen, U: MenuAccess<T> {
    ClientScreenRegistrationEvent.callback(Context(menu, supplier) as Context<in AbstractContainerMenu, Nothing>)
  }
}